...
if(isset($_SESSION['admin'])){
    if(isset($_POST['update']))
    {
        ...
        $requete = $bdd->query("UPDATE Bateau SET nomBateau = '$nom', prix = '$prix' WHERE idBateau = $idBateau");
    }

    if(isset($_POST['delete']))
    {
        $requete = $bdd->query("DELETE FROM Bateau WHERE idBateau =".$_POST['suppBateau']);
    }
?>
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-sm">
            ...
            <tbody> 
            <?php
                $products = $bdd->query("SELECT * FROM bateau");
            ...
            ?>
                <tr>
                    <td><?= $requete->idBateau?></td>
                    <td><?= $row->nom?></td>
                    <td><?= substr($row->description, -20)?> ...</td>
                    <td><?= $requete->prix ?> €</td>
                    <td>
                    <?php 
                        $requet = $bdd->query("SELECT * FROM Boutique WHERE id_bateau =".$requete->idBateau);

                        foreach($requet as $ro):
                            echo $ro->active;
                        endforeach;
                    ?>
                    </td>
